...
const form = {
	...
	*/
			...
			for (let i = 0, length = rule.length; i < length; i++) {
				...
				switch (ruleItem) {
					case "required":
						...
						break;
					case "arrRequired":
						isError = form._isArrRequired(formData[key]);
						break;
					case "isMobile":
						isError = !form._isMobile(formData[key]);
						break;
					case "isMobileAndIsEmail":
						isError = !form._isMobileAndIsEmail(formData[key]);
						break;
					case "isEmail":
					...
					case "maxLength":
						...
						break;
					case "neq":
						isError = (formData[key].toString() != value) ? false : true;
						break;
					default:
						break;
				}
	...
	_isNullOrEmpty: function(value) {
		return (value === null || value === '' || value === undefined || value === []) ? true : false;
	},
	_isArrRequired: function(value) {
	...
	_isMobileAndIsEmail: function(value) {
		return /^(?:13\d|14\d|15\d|16\d|17\d|18\d|19\d)\d{5}(\d{3}|\*{3})$/.test(value) || /^[a-z0-9]+([._\\-]*[a-z0-9])*@([a-z0-9]+[-a-z0-9]*[a-z0-9]+.){1,63}[a-z0-9]+$/.test(value);
	},
