		...
		methods: {
			...
			sendMsg() {
				...
				if (!phoneData) {
					...
					this.$Request.getT("/appLogin/sendMsg/" + phoneData + "/bind").then(res => {
						if (res.code === 0) {
							this.sending = true;
			...
			startReg() {
				...
				if (this.invitation != undefined && this.invitation != '') {
				} else {
					uni.showToast({
						icon: 'none',
						position: 'bottom',
				...
				this.$Request.postJson("/appLogin/loginWeiXinApp", {
					...
					if (res.code === 0) {
						if (uni.getSystemInfoSync().platform == "android") {
							...
							this.$Request.postT('/appLogin/updateClientId?clientId=' + clientid + '&userId=' + res.userId).then(res => {
